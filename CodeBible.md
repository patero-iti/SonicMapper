...
| **v0.4.0** | *2026-09-16* | Phase 2: Mobile Orientation & Field Soundwalks | Hardware compass orientation binding (`DeviceOrientationEvent` + iOS permission flow), dynamic cartographic heading cone on listener pin, real-time 360° Ambisonic FOA soundfield yaw rotation matrix linked to listener head direction, adaptive GPS drift & jitter smoothing filter (`GeoEngine.smoothCoordinates`), acoustic hysteresis geofence buffers, Screen Wake Lock API (`navigator.wakeLock`), and Media Session API lock screen integration. |
| **v0.5.0** | *2026-09-16* | Phase 3: Sound Art & Archival Tooling | Native GeoJSON Polygon geofence zones (river estuary shorelines and wetland reserves) with ray-casting Point-in-Polygon calculations and exterior buffer distance attenuation; multi-track spatial crossfading matrix for overlapping habitat biomes; multi-mode canvas visualizer supporting Oscilloscope Waveform, FFT Spectrum Bars, and 2D Spectrogram waterfall with Radio DADAA 4-tier colormap; archival audio asset downloader supporting uploaded IndexedDB blobs, remote media streams, and 16-bit 48kHz PCM WAV bounce synthesis. |
| **v0.6.0** | *2026-09-17* | Phase 4: Open Audio Cartography Protocol, PWA & Soundwalks | Full Progressive Web App (PWA) installation via `manifest.webmanifest`; Service Worker (`sw.js`) cache-first runtime engine caching app shell, audio binaries, and vector map tiles for offline fieldwalks in remote nature reserves; live Network Status HUD indicator; native GeoJSON `LineString` soundwalk trajectory paths with numbered waypoint markers, continuous path distance attenuation, and waypoint pacing; Open Audio Cartography Protocol (OACP) standalone archival package bundler with base64 binary audio serialization; and fullscreen drag-and-drop viewport ingestion. |
| **v0.7.0** | *2026-09-18* | Phase 4+: Polygon Geofence Authoring & Vertex Reshaping | Interactive on-map drawing mode for Polygon Geofence Zones with dynamic vertex nodes, real-time perimeter / area metrics, and rubber-band vector preview; Dedicated Polygon Creation modal with custom fade buffer (10m–200m), procedural ambient synthesizer presets, audio file upload, and habitat taxonomy; Interactive on-map boundary reshaping engine with draggable vertex handles, midpoint `+` vertex insertion, right-click vertex deletion, and live centroid recalculation; Direct map click selection opening feature details and geometry stats. |
| **v0.8.0** | *2026-09-18* | Phase 4+: Soundwalk Path Trajectory Authoring & Reshaping | Interactive on-map drawing mode for Guided Soundwalk Paths (`LineString`) with real-time polyline rubber-band preview, sequential numbered waypoint node placement, and live spherical route distance calculation; Dedicated Soundwalk Creation Modal with corridor buffer radii (10m–150m), procedural soundscape ambience, and custom audio file attachment; Interactive on-map waypoint reshaping engine with draggable waypoint handles (`⚑ Trailhead`, `2`, `3`...), midpoint `+` waypoint insertion, and right-click waypoint deletion; Direct map click selection of soundwalk route lines with playback drawer diagnostics and automatic IndexedDB persistence. |
| **v0.9.0** | *2026-09-20* | Phase 4+: Dual Audio Layering & Multi-Waypoint Soundwalk Mixing | Dual Audio Layering on soundwalks allowing simultaneous continuous Synthetic Ambience Bed (wind, water, birdsong, urban) AND primary trail audio recordings with independent master level faders; Multi-Waypoint Stem Audio Mixer allowing individual waypoints along a path to each host distinct audio stems or procedural synth presets, trigger radii, and volume controls with real-time crossfading; Interactive Soundwalk Multi-Track Mixer Deck and Collapsible Waypoint Stem Deck in Playback Drawer with live distance HUD, in-situ file attachments, and solo waypoint auditioning. |
